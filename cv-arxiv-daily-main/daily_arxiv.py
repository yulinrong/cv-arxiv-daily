...
def get_daily_papers(topic,query="slam", max_results=2):
    ...
    for result in search_engine.results():
        ...
        #pdf_url = arxiv_url + 'pdf/' + paper_key # 可以下载论文
        pdfs_dir = './pdfs'
        pdf_url = arxiv_url + 'pdf/' + paper_key + '.pdf'
        pdf_path = os.path.join(pdfs_dir, paper_key + '.pdf')  # 构建 PDF 文件的本地路径
        pngs_dir = './pngs'
        md_dir = './mds'
        ...
        try:
            ...
            logging.info(f"Downloaded {paper_title} to {pdf_path}")
        except requests.exceptions.RequestException as e:
            logging.error(f"Failed to download {paper_title}. Error: {e}")
        ...
        try:
            # PDF 转换为图片
            pages = convert_from_path(pdf_path, first_page=1, last_page=1)  # 只转换第一页
            if pages:
                first_page = pages[0]
                image_path = os.path.join(pngs_dir, paper_key + '.png')
                first_page.save(image_path, 'PNG')
                logging.info(f"Saved first page of {paper_title} as image to {image_path}")
        except Exception as e:
            logging.error(f"Failed to convert {paper_title} to image. Error: {e}")
...
def save_to_csv(data_collector, csv_path):
    fieldnames = ['Title', 'Authors', 'Year', 'URL', 'PDF_Path']  # 定义CSV的列名
